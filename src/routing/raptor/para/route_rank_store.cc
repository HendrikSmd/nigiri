...
auto skip_list_route_rank_store::cista_members() {
  return std::tie(route_ranks_,
                  scan_stop_starts_,
                  scan_stops_,
                  partition_);
...
cista::wrapped<skip_list_route_rank_store> skip_list_route_rank_store::read(std::filesystem::path const& path) {
  return cista::read<skip_list_route_rank_store>(path);
}

void skip_list_route_rank_store::write(std::filesystem::path const& path) const {
  return cista::write(path, *this);
}

void skip_list_route_rank_store::digest(timetable const& tt, route_partition partition, vecvec<route_idx_t, rank_t> route_event_ranks) {
  route_ranks_.clear();
  route_ranks_.resize(tt.n_routes(), rank_t{0U});
  for (auto route_idx = route_idx_t{0U}; route_idx < tt.n_routes(); ++route_idx) {
    auto max_rank = rank_t{0U};
