...
struct para_search {
  ...
  para_raptor init(raptor_state& raptor_state,
    ...
    utl::verify(q_.via_stops_.empty(),
                "too many via stops: {}, limit: {}", q_.via_stops_.size(),
                0U);
    state_.is_via_.fill(bitvec(tt_.n_locations()));
  ...
  para_search(timetable const& tt,
      ...
      : tt_{tt},
        ...
        search_interval_{std::visit(
            ...
            q_.start_time_)},
        fastest_direct_{get_fastest_direct(tt_, q_, direction::kForward)},
        algo_{init(raptor_state, rank_store)},
        timeout_(timeout) {
    utl::sort(q_.start_);
  ...
  routing_result execute() {
    ...
    auto const itv_est = interval_estimator<direction::kForward>{tt_, q_};
    if (is_pretrip()) {
    ...
    while (true) {
      ...
      if (new_interval.to_ != search_interval_.to_) {
        add_start_labels(interval{search_interval_.to_, new_interval.to_}, true);
        trace("dir=FWD, interval extension later -> reset state\n");
    ...
    if (is_pretrip()) {
      ...
      if (q_.slow_direct_) {
        ...
        for (auto const& j : state_.results_) {
          ...
          get_direct(tt_, nullptr, l.from_,l.to_,
                     q_, search_interval_, direction::kForward, done, direct);
        }
...
private:
  ...
  void add_start_labels(start_time_t const& start_interval,
    ...
    get_starts(direction::kForward, tt_, nullptr, start_interval, q_.start_, q_.td_start_,
               q_.via_stops_, q_.max_start_offset_, q_.start_match_mode_,
               q_.use_start_footpaths_, state_.starts_, add_ontrip, q_.prf_idx_,
               q_.transfer_time_settings_, bitvec::max(tt_.n_routes()));
    std::sort(
        begin(state_.starts_), end(state_.starts_),
        [&](start const& a, start const& b) { return b < a; });
  }

  void remove_ontrip_results() const {
    utl::erase_if(state_.results_, [&](journey const& j) {
      return !search_interval_.contains(j.start_time_);
    });
  }
    ...
    utl::equal_ranges_linear(
        ...
        [&](auto&& from_it, auto&& to_it) {
          ...
          auto const worst_time_at_dest =
              start_time + (std::min(fastest_direct_, q_.max_travel_time_) + duration_t{1});
