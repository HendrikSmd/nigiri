...
struct routing_time {
  ...
  friend bool operator>=(routing_time const&, routing_time const&) = default;
  friend bool operator>(routing_time const&, routing_time const&) = default;
  constexpr std::int32_t t() const { return offset_; }
  constexpr day_idx_t day() const { return day_idx_t{offset_ / 1440}; }
  constexpr minutes_after_midnight_t mam() const {
    return minutes_after_midnight_t{offset_ % 1440};
  }
  constexpr std::pair<day_idx_t, minutes_after_midnight_t> day_idx_mam()
      const {
    return {day(), mam()};
  }
  constexpr routing_time operator+(duration_t const& rt) const {
    return routing_time{(offset_ + rt.count())};
  }
  constexpr routing_time operator-(duration_t const& rt) const {
    return routing_time{(offset_ - rt.count())};
  }
...
inline constexpr auto kInvalidTime = SearchDir == direction::kForward
                                               ? routing_time::max()
                                               : routing_time::min();
